...
mock_logs = [
    {"timestamp": datetime.now().isoformat(), "level": "info", "message": "Robot service started", "kubernetes": {"pod_name": "robot-service", "container_name": "robot-service"}},
    {"timestamp": (datetime.now() - timedelta(minutes=5)).isoformat(), "level": "info", "message": "Robot added: test-robot-efk", "kubernetes": {"pod_name": "robot-service", "container_name": "robot-service"}},
    {"timestamp": (datetime.now() - timedelta(minutes=10)).isoformat(), "level": "warning", "message": "High CPU usage detected", "kubernetes": {"pod_name": "prometheus", "container_name": "prometheus"}},
    {"timestamp": (datetime.now() - timedelta(minutes=15)).isoformat(), "level": "error", "message": "Failed to connect to database", "kubernetes": {"pod_name": "robot-service", "container_name": "robot-service"}},
    {"timestamp": (datetime.now() - timedelta(minutes=20)).isoformat(), "level": "info", "message": "Dashboard service started", "kubernetes": {"pod_name": "dashboard", "container_name": "dashboard"}},
]
...
def parse_log_line(line):
    ...
    try:
        ...
        std_pattern = r'(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) (INFO|WARNING|ERROR|DEBUG) (.*)'
...
async def get_logs(
    ...
    with request_duration.labels(endpoint="/logs").time():
                    ...
                    if pod_item.spec and pod_item.spec.containers:
                        for container_item in pod_item.spec.containers:
                            ...
                            try:
                                ...
                                logger.error(f"Error getting logs for pod {pod_name}, container {container_name}: {e}")
        ...
        if not all_logs:
            ...
            if query:
                all_logs = [log for log in all_logs if query.lower() in log.get("message", "").lower()]
            
            if level:
                all_logs = [log for log in all_logs if log.get("level") == level]
            
            if pod:
                all_logs = [log for log in all_logs if log.get("kubernetes", {}).get("pod_name") == pod]
            
            if container:
                all_logs = [log for log in all_logs if log.get("kubernetes", {}).get("container_name") == container]
...
async def get_pods():
    with request_duration.labels(endpoint="/pods").time():
        ...
        pods = list(set(log.get("kubernetes", {}).get("pod_name") for log in mock_logs if log.get("kubernetes", {}).get("pod_name")))
        return {"pods": pods}
...
async def get_containers():
    with request_duration.labels(endpoint="/containers").time():
        ...
        containers = list(set(log.get("kubernetes", {}).get("container_name") for log in mock_logs if log.get("kubernetes", {}).get("container_name")))
